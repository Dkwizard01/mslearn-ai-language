    ...
    async def start(self):
        ...
        try:
            ...
            async with connect(
                ...
                self.audio_processor.loop = asyncio.get_event_loop()
...
class AudioProcessor:
    ...
    def _audio_sender_loop(self):
        """Consume audio from queue and send to VoiceLive (runs in background thread)."""
        while True:
            try:
                audio_base64 = self.input_queue.get(timeout=1)
                if audio_base64 is None:  # Shutdown signal
                    break
                asyncio.run_coroutine_threadsafe(
                    self.connection.input_audio_buffer.append(audio=audio_base64),
                    self.loop
